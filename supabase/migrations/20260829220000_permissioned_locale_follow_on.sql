-- Add a guarded, idempotent Japanese follow-on for permissioned dossiers.
-- Translated content is staged as machine_draft and must pass the existing
-- locale-specific QA state machine and publication contract before serving.
...
begin
  if p_locale <> 'ja' then
    raise exception 'unsupported_locale: permissioned follow-on currently supports ja only';
  end if;
