...
create policy deal_locales_select on public.deal_locales
  for select to authenticated
  using (
...
revoke all on table public.deal_locales from authenticated;
grant select on table public.deal_locales to authenticated;
...
create policy qa_assignments_select on public.qa_assignments
  for select to authenticated
  using (
    ((select public.is_platform(array['super_admin', 'publisher', 'editor', 'language_reviewer_ja', 'language_reviewer_ko', 'ops_agent'])))
  );

revoke all on table public.qa_assignments from authenticated;
grant select on table public.qa_assignments to authenticated;

-- ── qa_work_sessions ────────────────────────────────────────────────────

create policy qa_work_sessions_select on public.qa_work_sessions
  for select to authenticated
  using (
    ((select public.is_platform(array['super_admin', 'publisher', 'editor', 'language_reviewer_ja', 'language_reviewer_ko', 'ops_agent'])))
  );

revoke all on table public.qa_work_sessions from authenticated;
grant select on table public.qa_work_sessions to authenticated;
