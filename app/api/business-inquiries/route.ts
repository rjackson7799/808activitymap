...
import { submitBusinessInquiry } from "@/lib/business-inquiries/server";
...
import { isValidSessionId, SESSION_COOKIE } from "@/lib/analytics/session";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const parsed = businessInquirySchema.safeParse(await request.json().catch(() => null));
    ...
    const { per_ip, per_session } = config.business_inquiry_rate_limits;
    ...
    const result = await submitBusinessInquiry(parsed.data);
    if (!result.ok) return json(result.reason, 503);

    return NextResponse.json({ ok: true, reference: result.id.slice(0, 8) }, { status: 201 });
  } catch (error) {
    captureError(error, { where: "POST /api/business-inquiries" });
